<template>
  <section>
    ...
    <div v-for="v in pathList" :key="v" class="io">
      ...
      <button @click="openDirectory(settings[v])">{{ settings[v] }}</button>
  ...
  <section>
    ...
    <div v-for="v in sidecarList" :key="v" class="io">
      ...
      <button @click="openDirectory(settings.sidecar[v])">
        {{ settings.sidecar[v] }}
      </button>
...
async function getSize(type: CacheKey) {
  ...
  const result = await commands.getSize(type, event);
  if (result.status === 'error') {
    AppLog(
      `Failed to get cache size (${type}): ${result.error.message}`,
      'warning',
      { toast: false },
    );
  }
...
async function cleanCache(type: CacheKey) {
  ...
  if (!result) return;
  const clean = await commands.cleanCache(type);
  if (clean.status === 'error') {
    AppLog(
      `Failed to clean cache (${type}): ${clean.error.message}`,
      'warning',
      { toast: false },
    );
  }
...
async function openCache(type: CacheKey) {
  const result = await commands.openCache(type);
  if (result.status === 'error') {
    AppLog(
      `Failed to open cache (${type}): ${result.error.message}`,
      'warning',
      { toast: false },
    );
  }
...
async function openDirectory(path: string) {
  try {
    ...
    AppLog(
      `Failed to open path ${path}: ${e instanceof Error ? e.message : String(e)}`,
      'warning',
      { toast: false },
    );
  }
