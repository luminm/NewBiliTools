...
pub static CONFIG: LazyLock<ArcSwap<Settings>> = LazyLock::new(|| {
    ArcSwap::new(Arc::new(Settings {
        ...
        format: SettingsFormat {
            ...
            file: "{index} - {taskType} - {title}".into(),
        },
        ...
        max_conc: 3,
        // Keep a few logical cores reserved for the rest of the system.
        max_ffmpeg: std::thread::available_parallelism()
    ...
    for attempt in 1..=3 {
        ...
        match result {
            ...
            Err(e) if is_transient_error(&e) => {
                log::warn!("Failed to fetch image (attempt {attempt}/3): {e}");
                last_error = Some(e);
                sleep(Duration::from_millis(300 * attempt)).await;
            }
            Err(e) => return Err(e),
        }
    }
    Err(last_error.unwrap_or_else(|| TauriError::new("Failed to fetch image", None::<isize>)))
}

fn is_transient_error(err: &TauriError) -> bool {
