...
export function setEventHook() {
  ...
  events.processError.listen((e) => {
    const err = e.payload;
    const appError = new AppError(err.error, {
      name: `ProcessError (${err.name})`,
      transient: true,
    });
    AppLog(appError.message, 'warning', { toast: false });
  });
}
...
export async function tryFetch(
  ...
  const retries = options?.times ?? DEFAULT_RETRIES;
  let requestUrl = url;
  for (let i = 0; i < retries; i++) {
    try {
      ...
      requestUrl = url.replace('http:', 'https:') + params;
      ...
      if (!contentType.includes('application/json')) {
        ...
        throw new AppError(
          `Expected JSON response from ${requestUrl}, got ${response.status} ${response.statusText} (${contentType || 'no content-type'})${text ? `: ${text.slice(0, 160)}` : ''}`,
      ...
      if (body.code === -352 && body.data && i < retries) {
        const csrf =
      ...
      if (i < retries - 1) {
        const delay = 300 * (i + 1) + getRandomInRange(0, 200);
        AppLog(
          `Retrying request ${requestUrl} (${i + 2}/${retries}): ${e instanceof Error ? e.message : String(e)}`,
          'warning',
          { toast: false },
        );
      ...
      throw e instanceof AppError ? e : new AppError(e, { name: 'AppError' });
    } finally {
...
export function getDefaultQuality(
  ...
  if (ids.includes(quality[name])) return quality[name];

  const order = {
    res: [127, 126, 125, 120, 116, 112, 80, 64, 32, 16, 6],
    abr: [30252, 30251, 30250, 30380, 30280, 30232, 30228, 30216],
    enc: [13, 12, 7],
  } as const;
  return order[name].find((id) => ids.includes(id)) ?? ids[0];
}
