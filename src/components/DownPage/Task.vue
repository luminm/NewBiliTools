<template>
  ...
  >
    ...
    <div class="flex w-full gap-2 *:shrink-0 items-center">
      ...
      <span class="w-14">{{ progress.toFixed(2) }}%</span>
...
function getProgress(task: Types.Task) {
  ...
  let chunk = 0;
  const status: Types.Task['status'] = queue.tasks[task.id]?.status ?? {};
  for (const subtask of subtasks) {
    const value = status[subtask.id];
    if (value?.content) {
      chunk += Math.min(1, value.chunk / value.content);
    }
  }
  return content ? (chunk / content) * 100 : 0;
}

const progress = computed(() => getProgress(props.task));
