<template>
  ...
  <section>
    <h3>
      <i :class="[$fa.weight, 'fa-clapperboard']"></i>
      <span>{{ $t('settings.max_ffmpeg.name') }}</span>
    </h3>
    <Dropdown
      v-model="settings.max_ffmpeg"
      :drop="Array.from({ length: 20 }, (_, i) => ({ id: i + 1, name: i + 1 }))"
    />
    <span class="desc">{{ $t('settings.max_ffmpeg.desc') }}</span>
  </section>
  <hr />
  <section>
    <h3>
