...
async fn get_duration(req: &SubTaskReq, path: &Path) -> Result<u64> {
    if let Some(duration) = req.task.item.duration.as_u64() {
        if duration > 0 {
            return Ok(duration);
        }
    }
...
async fn probe_duration(path: &Path) -> Result<u64> {
    let app = get_app_handle();
...
pub async fn merge(
    ...
    let duration = get_duration(req, video).await?;
...
pub async fn add_meta(
    ...
    let app = get_app_handle();
    let duration = get_duration(req, input).await?;
