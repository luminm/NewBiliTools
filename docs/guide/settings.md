...
控制下载阶段同时进行的任务数量。合成与后处理阶段由 FFmpeg 合成并发数控制。

### 最大 FFmpeg 合成并发数

控制 ffmpeg 同时进行合成 / 转码的实例数量。该值与下载并发相互独立，因此批量下载时不会因为等待 ffmpeg 合成而暂停后续资源的下载。

默认值会根据当前机器的逻辑核心数自动计算，并保留少量核心给系统和其他程序。数值越高越吃 CPU；如果其他程序出现卡顿，可以手动调低该值。
