...
async function handleMedia(task: Types.Task) {
  ...
  let playUrl = await getPlayUrl(item, item.type, select.fmt);
  ...
  if (
    ...
    !playUrl.audio?.length
  ) {
    AppLog(
      'DASH audio unavailable, falling back to MP4: ' + item.title,
      'warning',
      { toast: false },
    );
    select.fmt = Types.StreamFormat.Mp4;
    playUrl = await getPlayUrl(item, item.type, select.fmt);
  }
  ...
  if (type === 'taskUpdated') {
    ...
    if (event.prepare) {
      ...
      task.status = Object.fromEntries(
        event.prepare.subtasks.map((subtask) => [
          subtask.id,
          task.status?.[subtask.id] ?? { content: 0, chunk: 0 },
        ]),
      );
    }
    if (event.cancelled) {
    ...
    queueProgress(event.task, event.subtask, event.content, event.chunk);
  } else if (type === 'queue') {
    queue[event.name] = event.value;
  } else if (type === 'request') {
    const task = queue.tasks[event.task];
    if (!task) throw new AppError(`Task#${event.task} not found`);
...
const pendingProgress = new Map<
  string,
  Map<string, { content: number; chunk: number }>
>();
...
function queueProgress(
  ...
  let subtasks = pendingProgress.get(task);
  if (!subtasks) {
    subtasks = new Map();
    pendingProgress.set(task, subtasks);
  }
  subtasks.set(subtask, { content, chunk });
  if (progressFlushScheduled) return;
  progressFlushScheduled = true;
  setTimeout(() => {
    progressFlushScheduled = false;
    const queue = useQueueStore();
    for (const [task, subtasks] of pendingProgress) {
      const statuses = queue.tasks?.[task]?.status;
      if (!statuses) continue;
      for (const [subtask, update] of subtasks) {
        const status = statuses[subtask];
        if (status) {
          status.content = update.content;
          status.chunk = update.chunk;
        }
      }
    }
...
export async function processQueue() {
  try {
    ...
    scheduler.data.list.forEach((v) => {
      const task = queue.tasks[v];
      if (task) task.sid = scheduler.data.sid;
    });
